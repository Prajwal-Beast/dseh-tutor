...
- If the student asks about something outside the four subject areas, politely say it is out of scope.`;
...
export function buildTutorSystemPrompt(syllabusTopics: SyllabusTopic[]): string {
  ...
  const block = syllabusTopics
    .map((t) => `[${t.subject}${t.subtopic ? ` > ${t.subtopic}` : ''}]\n${t.content.slice(0, 600)}`)
    .join('\n\n---\n\n');
...
4. The explanation must state WHY the correct answer is right and why the main distractors are wrong.
5. Return ONLY valid JSON – no markdown fences, no extra text – using exactly this schema:
